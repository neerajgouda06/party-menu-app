...
export const filterMenuItems = ({ category = 'all', name = '', diet = 'all' }) => {
  return menuItems.filter(item => {
    const matchCategory = category === 'all' || item.category === category;
    const matchName = !name || item.name.toLowerCase().includes(name.toLowerCase());
    const matchDiet = diet === 'all' || (diet === 'veg' && item.isVeg) || (diet === 'nonveg' && !item.isVeg);
    return matchCategory && matchName && matchDiet;
  });
};
